% Robust Attitude Estimation via Single Tuning Factors (STFs) using IMU-Only Measurements 
...
% R-Adaptation via Novel Single Tuning Factor (STF) Method
ek = e ;                                % Single 
S = (ek'*ek-trace(H*P*H')-trace(R))/3 ; % Tuning
S = max(S,0) ;                          % Factor Construction 

% Measurement Update (Correction)
K = P*H'*inv(H*P*H'+S*eye(3)+R) ;       % Kalman Gain Evaulation
